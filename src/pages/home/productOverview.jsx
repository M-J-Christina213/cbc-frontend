...
export default function ProductOverview() {
  ...
  const [newReview, setNewReview] = useState({ name: "", email: "", rating: 5, review: "" });  // Define newReview state
  ...
  const submitReview = async () => {
    try {
      await axios.post("/api/reviews", { ...newReview, productID });
  ...
  return (
    <div className="w-full h-[calc(100vh-190px)]">
      ...
      {status === "found" && (
        <div className="w-full h-full flex flex-col lg:flex-row items-center justify-center p-8">
          ...
          <div className="w-full md:w-[65%] h-300px p-6 relative">
            ...
            <button onClick={onAddtoCartClick} className="bg-primary text-white p-2 ">
              Add to Cart
            </button>
            <button onClick={onBuyNowClick} className="text-primary border-primary border mx-1 p-2">
              Buy now
            </button>
            
            {/* Review Section */}
            <div className="mt-10 relative bg-gradient-to-r from-pink-300 via-purple-300 to-indigo-400 p-6 ">
